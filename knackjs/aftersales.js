...
$(document).on('knack-view-render.view_2686', function(event, view, records) {
  ...
  if ($('div[id="view_2686"] div[class="kn-records-nav"]>div[id*="syButtons"]').length===0){
    let buttonsDiv = '<div id="syButtons_2686" class="js-filter-menu tabs is-toggle is-flush"><ul>';
    buttonsDiv += defineButtons.map((el,index) =>'<li><a id="syButtons_2686_'+index+'" onclick="syButtonsFilter(2686,'+index+','+el.linkText+','+el.filters+'); return false;";><span>'+el.linkText+'</span></a></li>').join('');
    buttonsDiv += '</ul></div>';
