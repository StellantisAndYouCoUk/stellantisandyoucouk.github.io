...
//C/D confirm bay motability photo
$(document).on('knack-view-render.view_3222', function (event, view, data) {
  embedPhotoApp();
  let appSettings2718 = {
    spiritLine : false,
    imageOverlay: 'https://stellantisandyoucouk.github.io/imagesStore/car-background-v2.png',
  ...
  formButton.onclick = function() {
    if (!isOnline){
      ...
      if ($('input[imageToSaveUrl]').length>0 || $('input[id*="offline"]').length>0){
        uploadList = [];
        $('div[id="view_3222"] button[type="submit"]').prop('disabled', true);
        $('<h3>Images are being uploaded, then the form will be submitted ...</h3><p id="infoText"></p>').insertBefore($('div[id="view_3222"] button[type="submit"]'))
        createFormModal('fMImageUpload','<h3>Images and files are being uploaded, then the form will be submitted ...</h3><p id="infoText"></p>');
        $('#fMImageUpload').show();
        for (let i =0;i<$('input[imageToSaveUrl]').length;i++){
          uploadList.push({field:$('input[imageToSaveUrl]').eq(i).attr('name')})
