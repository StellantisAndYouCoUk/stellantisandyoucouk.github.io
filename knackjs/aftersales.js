...
$(document).on('knack-view-render.view_2686', function(event, view, records) {
  ...
  if ($('div[class="kn-records-nav"]>div[id="syButtons"]').length===0){
    let buttonsDiv = '<div class="js-filter-menu tabs is-toggle is-flush"><ul>';
    buttonsDiv += defineButtons.map((el,index) =>'<li><a id="'+index+'"><span>'+el.linkText+'</span></a></li>').join('');
    buttonsDiv += '</ul></div>';
