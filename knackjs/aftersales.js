...
function availabilityHTML(status){
  ...
  if (status.addressData && status.addressData.closestD){
    for (let i = 0;i<status.addressData.closestD.length;i++){
      ...
      if (avail) htmlTable += '<tr><td>'+status.addressData.closestD[i].name.replace('Stellantis &You','')+'</td><td>'+status.addressData.closestD[i].duration+(status.lastVisitData && status.addressData.closestD[i].companyCode===status.lastVisitData.mapLastDealerVisit?', Last Visited':'')+'</td><td>'+formatDateGBShort(new Date(avail.work.find(el=>el.work==='MOT').availability))+'</td><td>'+formatDateGBShort(new Date(avail.work.find(el=>el.work==='Recall').availability))+'</td><td>'+formatDateGBShort(new Date(avail.work.find(el=>el.work==='Small service').availability))+'</td><td>'+formatDateGBShort(new Date(avail.work.find(el=>el.work==='Large service').availability))+'</td></tr>';
  ...
  if (status.lastVisitData && status.lastVisitData!=='' && !lastVisitedInClosest){
    ...
    if (avail) htmlTable += '<tr><td>'+status.lastVisitData.lastDealerVisit.replace('Stellantis &You','')+'</td><td>Last visited</td><td>'+formatDateGBShort(new Date(avail.work.find(el=>el.work==='MOT').availability))+'</td><td>'+formatDateGBShort(new Date(avail.work.find(el=>el.work==='Recall').availability))+'</td><td>'+formatDateGBShort(new Date(avail.work.find(el=>el.work==='Small service').availability))+'</td><td>'+formatDateGBShort(new Date(avail.work.find(el=>el.work==='Large service').availability))+'</td></tr>';
  }
  htmlTable += '</table>';
