...
document.addEventListener('knack-view-render.view_3773', function(event) {
  console.log("View render event triggered");
  ...
  Promise.all([
    ...
    head.append(sweetAlertCSS);

    // Add Modal Structure to the body
    document.addEventListener('mouseleave', function(event) {
      if (event.target.classList.contains('fa-cart-arrow-down')) {
        // Remove any existing modal with the same ID to prevent duplicates
        ...
        // Append the modal structure to the body
        fetch('https://stellantisandyoucouk.github.io/modalHTML/modal.html')
          .then(response => response.text())
          .then(partsModelHTML => {
            document.body.insertAdjacentHTML('beforeend', partsModelHTML);
