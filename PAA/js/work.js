...
function work(){
    ...
    if (page.includes('runs.html')){
        ...
        let tM = req.map(function (el){
            return '<tr><td>'+el.flowName+'</td><td>'+el.status+'</td><td>'+el.priority+'</td><td>'+ dateTimeToGB(new Date(el.createdDateTime))+'</td><td>'+ dateTimeToGB(new Date(el.startedDateTime))+'</td><td>'+ dateTimeToGB(new Date(el.completedDateTime))+'</td><td></td></tr>';
        })
        console.log(tM[0]);
