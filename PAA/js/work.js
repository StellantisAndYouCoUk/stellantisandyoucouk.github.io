...
function work(){
    ...
    if (page.includes('runs.html')){
        ...
        let tM = req.map(function (el){
            return '<tr><td>'+el.flowName+'</td><td'+(el.status==='succeded'?' class="bg-success"':'')+'>'+el.status+'</td><td>'+el.priority+'</td><td>'+ dateTimeToGB(new Date(el.createdDateTime))+'</td><td>'+ (el.startedDateTime?dateTimeToGB(new Date(el.startedDateTime)):'')+'</td><td>'+ (el.completedDateTime&&el.startedDateTime?(new Date(((new Date(el.completedDateTime)-new Date(el.startedDateTime))/60000)* 1000).toISOString().substring(14, 19)):'')+'</td><td><button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#runDetails" onclick="fillModal(\'runDetailsBody\',\'runDetailsText-'+el.runId+'\');">Show details</button>'+formatRunDetails(el)+'</td><td><a target="_blank" href="'+el.hrefDetails+'">Open</a></td></tr>';
        })
        //console.log(tM[0]);
