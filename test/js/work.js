...
async function work(){
    ...
    document.getElementById('compute').addEventListener('click', () => {
        ...
        let tM = data.wwoofersByCountry.map(function (el){
            return '<tr><td>'+el.country+'</td><td>'+Math.round(el.totalWWOOFers)+'</td><td>'+el.percentZero+'%</td><td>'+el.totalIncome+' EUR</td><td>'+Math.round(el.globalMembershipTotal)+' EUR</td><td>'+el.globalMembershipLocalMemberships+' EUR</td><td>'+el.globalMembershipShareOfGlobalMemberships+' EUR</td><td>'+el.globalMembershipsAddedVisitsCount+'</td><td>'+Math.round(el.globalMembershipsAddedVisitsSum)+' EUR</td><td>'+el.surplusDistribuiton+' EUR</td></tr>';
        })
